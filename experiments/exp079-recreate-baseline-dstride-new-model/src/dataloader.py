...
class EziiDataset(Dataset):
        ...
        for exp_name in exp_names:
            for type_ in zarr_type:
                if len(exp_name.split("_")) == 2: # 追加データセットならば
                    self.data.append((exp_name, "none"))
                    break
                else:
    ...
    def __getitem__(self, i):
        if self.train:
            if self.pre_read:
                ...
                normalized_tomogram = self.data_dict[f"{exp_name}_{type_}"]["normalized_tomogram"]
                tomogram = self.data_dict[f"{exp_name}_{type_}"]["tomogram"]
                segmentation_map = self.data_dict[f"{exp_name}_{type_}"]["segmentation_map"]
                zarr_type = type_
            else:
                exp_name, type_ = self.data[i]  # TS_6_6
            ...
            zarr_type_embedding_idx = CFG.zarr2idx[zarr_type]
            
            return {
                "exp_name": exp_name,
                "zarr_type": zarr_type,
                "zarr_type_embedding_idx": zarr_type_embedding_idx,
                "resolution": self.resolution,
                "tomogram": tomogram,
            ...
            normalized_tomogram, min, max = normalise_by_percentile(tomogram)
            zarr_type_embedding_idx = CFG.zarr2idx[type_]
            return {
                "exp_name": exp_name,
                "zarr_type": type_,
                "zarr_type_embedding_idx": zarr_type_embedding_idx,
    ...
    def pre_read_f(self):
        for i in tqdm(range(len(self.data))):
            ...
            if self.data_dict.get(f"{exp_name}_{type_}") is not None:
                continue
            ...
            self.data_dict[f"{exp_name}_{type_}"] = {
                "exp_name": exp_name,
                "resolution": self.resolution,
                "tomogram": tomogram,
                "normalized_tomogram": normalized_tomogram,
