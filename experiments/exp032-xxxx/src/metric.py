...
def create_cls_pos(pred_tomogram):
    ...
    for pred_cls in range(1, len(CFG.particles_name) + 1):
        cc = cc3d.connected_components(pred_tomogram == pred_cls, connectivity=2)
...
def create_cls_pos_sikii(pred_tomogram, sikii_dict=CFG.initial_sikii):
    ...
    for pred_cls in range(1, len(CFG.particles_name) + 1):
        ...
        # メモリを節約しながら接続成分解析を実行
        cc = cc3d.connected_components(cls_tomogram == pred_cls, connectivity=6)
        stats = cc3d.statistics(cc)
