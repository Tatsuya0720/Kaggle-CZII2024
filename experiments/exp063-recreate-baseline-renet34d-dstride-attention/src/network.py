...
class CrossDomainAttention(nn.Module):
    ...
    def __init__(self, embed_dim=128, num_heads=4, num_domains=5):
        ...
        # conv2d層を使って、ドメイン埋め込みを特徴マップに合わせる
        self.conv1 = nn.Conv2d(embed_dim*2, embed_dim, kernel_size=1, bias=False)
    ...
    def forward(self, x, domain_idx):
        ...
        domain_embeds = domain_embeds.reshape(B, C, 1, 1, 1).expand(-1, -1, D, H, W)

        x = torch.cat([x, domain_embeds], dim=1)  # => (B, C*2, D, H, W)
        x = x.transpose(1, 2).reshape(B*D, C*2, H, W)  # => (B*D, C*2, H, W)
        x = self.conv1(x)  # => (B*D, C, H, W)
        x = x.reshape(B, D, C, H, W).transpose(1, 2)  # => (B, C, D, H, W)
        # x_out = x_reshaped.view(B, C, D, H, W) # => (B, C, D, H, W)
