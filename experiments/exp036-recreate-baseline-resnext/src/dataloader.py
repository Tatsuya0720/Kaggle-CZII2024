...
class EziiDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if particle_info is not None:
            ...
            # random=Trueならば，tomogram, normalized_tomogram, segmentation_mapの0次元目をランダムにしゃっふる
            
            if self.random:
                random_index = np.random.permutation(tomogram.shape[0])
                tomogram = tomogram[random_index]
                normalized_tomogram = normalized_tomogram[random_index]
                segmentation_map = segmentation_map[random_index]
